  ...
  "your","yourself","youth","zero","zebra","zipper","zulu"
    ...
    //game data object contains all the variables and functions except for the word picker and key events
    var gameData = {
      wins: 0,
      losses: 0,
      guesses: 12,
      ...
      {
        var html =
              "<h2>You chose: " + gameData.userGuesses.join() + "</h2>" +
              "<h3>Wins: " + gameData.wins + " Losses: "+gameData.losses+"</h3>" +
              "<h3>Guesses left: " + gameData.guesses + "</h3>" +
              "<h2>"+ gameData.answerArray.join(" ") +"</h2>";
       ...
       checkLetter: function(L)
  {
    var isCorrect = false;
      for(var i=0;i<gameData.computerGuess.length; i++)
      {
        if(L===gameData.computerGuess[i]){
          isCorrect=true;
          gameData.answerArray[i]=L;
          var finalAnswer = gameData.answerArray.join("");
          gameData.setGUI();
          if(finalAnswer === gameData.computerGuess)
          {
   ...
   gameData.userGuess = event.key;
      if(alphabet.indexOf(gameData.userGuess.toLowerCase())>=0)//if index is -1 it't not in the alphabet array and thus not a letter.
      {
        if(gameData.userGuesses.indexOf(gameData.userGuess)<0) //if index is -1 it hasn't been guessed yet and is a vaild guess
        {
          ...
          gameData.checkLetter(gameData.userGuess);
        }
  ...
  function pickWord()
  {
    gameData.computerGuess = computerChoices[Math.floor(Math.random() * computerChoices.length)];
    if(gameData.computerGuess.length!==6)pickWord(); // pick a six letter word. THis can later be changed to a variable. I could also make '<' and '> to use a range.
  }
