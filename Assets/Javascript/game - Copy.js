  ...
  document.onkeyup = function(event) {
   if(!gameStarted)return;
      userGuess = event.key;
      if(alphabet.indexOf(userGuess.toLowerCase())>=0)//if index is -1 it't not in the alphabet array and thus not a letter.
      {
  ...
  function pickWord()
  {
    ...
    if(computerGuess.length!==6)pickWord();
  }
  ...
  function checkLetter(L)
  {
    var isCorrect = false;
      for(var i=0;i<computerGuess.length; i++)
      {
        if(L===computerGuess[i]){
          isCorrect=true;
          answerArray[i]=L;
          var finalAnswer = answerArray.join("");
          setGUI();
          if(finalAnswer === computerGuess)
          {
